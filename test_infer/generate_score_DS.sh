...
# 定义固定路径
INPUT_DIR="./test_infer/generate_answer"
OUTPUT_DIR="./test_infer/generate_scores_DS" # 修改输出目录名称
PYTHON_SCRIPT="./test_infer/generate_score_DS.py" # 修改 Python 脚本名称
...
# 自动计算所有科室的均分
echo "开始计算所有科室的评分均分..."
python3 "$PYTHON_SCRIPT" --average --score_dir "$OUTPUT_DIR" # 传入正确的评分目录
if [ $? -ne 0 ]; then
    echo "错误：计算均分失败"
else
    echo "完成：均分已成功保存"
