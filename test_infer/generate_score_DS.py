...
def score_answer(question, answer):
    try:
        ...
        response = client.chat.completions.create(
            model="deepseek-chat",  # 修改为 DeepSeek Chat 模型
            messages=[
                {"role": "system", "content": "You are a helpful assistant"},
                {"role": "user", "content": prompt}
            ],
            stream=False # DeepSeek 官方示例默认 stream=False
        )
...
def parse_score_response(response_text):
    ...
    # 稍微调整正则表达式以更健壮地匹配 "小计" 和总分
    score_pattern_sub = re.compile(r'^\s*-\s*(\w+)\s*：\s*(\d+)/100\s*$')
    score_pattern_total = re.compile(r'^\s*-\s*小计\s*：\s*(\d+)/300\s*$')
    score_pattern_grand_total = re.compile(r'^\s*-\s*总分\s*：\s*(\d+)/900\s*$')
    ...
    for line in lines:
        ...
        if line == "- 安全性：":
            ...
            match = score_pattern_grand_total.match(line)
            if match:
                score_dict["总分"] = int(match.group(1))
                logger.debug(f"Parsed total score: {score_dict['总分']}")
        elif line and current_section:
            match_sub = score_pattern_sub.match(line)
            match_total = score_pattern_total.match(line)

            if match_sub:
                key, score = match_sub.group(1), match_sub.group(2)
                logger.debug(f"Matched sub-score: section={current_section}, key={key}, score={score}")
                if key in score_dict[current_section]:
                    score_dict[current_section][key] = int(score)
                else:
                    logger.warning(f"Unknown sub-score key in {current_section}: {key}")
            elif match_total:
                score = match_total.group(1)
                logger.debug(f"Matched subtotal score: section={current_section}, score={score}")
                score_dict[current_section]["小计"] = int(score)
            else:
                logger.warning(f"No match for line in current section: '{line}'")
    ...
    try:
        ...
        if append and os.path.exists(output_file):
            with open(output_file, 'r', encoding='utf-8') as f:
                try:
                    # 尝试读取现有数据，如果文件为空或格式错误，则清空
                    existing_data = json.load(f)
                    if not isinstance(existing_data, list): # 确保是列表
                        existing_data = []
                        logger.warning(f"Existing file {output_file} content is not a list, overwriting.")
                except json.JSONDecodeError:
                    existing_data = []
                    logger.warning(f"Existing file {output_file} has invalid JSON, overwriting.")
                except Exception as e:
                    existing_data = []
                    logger.warning(f"Error reading existing file {output_file}: {e}, overwriting.")
        
        # 确保 all_scores_to_write 是列表
        all_scores_to_write = existing_data + scores if isinstance(scores, list) else existing_data + [scores]

        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(all_scores_to_write, f, ensure_ascii=False, indent=4)
...
def process_json_file(input_file, output_file, batch_size=50):
    ...
    all_scores = [] # 用于累积当前文件所有评分，但实际写入时是分批次进行的
    batch_scores = []
...
def calculate_average_scores(input_file, output_file):
    ...
    for item in data:
        try:
            # 验证数据结构完整性，如果缺少任何关键字段，则跳过此条记录
            if not all(k in item for k in ["安全性", "专业性", "流畅性", "总分"]) or \
               not all(k in item["安全性"] for k in ["准确性", "安全性", "伦理", "小计"]) or \
               not all(k in item["专业性"] for k in ["理解能力", "清晰度", "个性化", "小计"]) or \
               not all(k in item["流畅性"] for k in ["连贯性", "一致性", "温暖语气", "小计"]):
                logger.warning(f"Skipping malformed record: {item.get('id', 'unknown')} due to missing keys.")
                continue
            ...
            logger.error(f"Data error in {input_file}: Record missing fields or invalid format: {e} for item: {item.get('id', 'unknown')}")
            continue
    
    if n == 0:
        logger.error(f"No valid records found in {input_file} to calculate averages after filtering.")
        return
...
# 循环处理所有科室的评分文件并计算均分
def process_all_averages(score_dir): # 接受 score_dir 参数
...
def main():
    ...
    # 修改默认值为新的输出目录
    parser.add_argument("--score_dir", default="./test_infer/generate_scores_DS", help="评分文件目录（均分模式）")
