...
@dataclass
class KeyEncoderConfig:
    output_dir: str = field(
        default='lora/',
        metadata={
            "help": "The output directory where the lora checkpoints and logging files will be written."
        }
    )
    seed: int = field(default=0, metadata={"help": "Random seed that will be set at the beginning of training."})
    task_list: str = field(
    ...
    global_composition: bool = field(
        default=True,
        metadata={
            "help": "LoRA modules are composed at the global level (True) or layer-wise (False) "
        }
    )
    reinit_cls: bool = field(
        default=True,
        metadata={
            "help": "Reinit the classifier (score) layer for each task (required for the case that each task has different label space)."
        }
    )
    num_labels_cur: int = field(
        default=None,
        metadata={
            "help": "The number of labels for the current task, for re-initialize the classifier."
        }
    )
    key_dim: int = field(
        default=None,
        metadata={
            "help": "Dimension of the task key vectors, should be the same with the model hidden size."
        }
    )
    query_encoder_type: str = field(
        default="avg_word_embed",
        metadata={
            "help": "Query embedding types, options: ['last_hidden_states', 'avg_all_embed', 'avg_word_embed']"
        }
    ) 
    direct_compose: bool = field(
        default=False,
        metadata={
            "help": "Directly use query-key cosine_similarity for module composition and matching loss calculation (if required)."
        }
    )
    ...
    key_init_func: str = field(
        default="uniform",
        metadata={
            "help": "Initialize key vectors with 'orthogonal' / 'uniform' nn.init functions "
        }
    )
    ortho_loss_key: bool = field(
        default=False,
        metadata={
            "help": "Add orthogonal loss/penalty of keys or not"
        }
    ) 
    ortho_loss_coeff: float = field(
        default=0.1,
        metadata={
            "help": "Coefficient for the orthogonal loss/penalty term"
        }
    ) 
    key_ortho_mode: str = field(
        default="unnormalized_soft",
        metadata={
            "help": "Orthogonality mode for task vectors: unnormalized_soft, hard, cosine_soft."
        }
    )
    key_ortho_threshold: float = field(
        default=0.1,
        metadata={
            "help": "Soft threshold used by task-vector orthogonality regularization."
        }
    )
    lamda_keys_L2: float = field(
        default=0.0,
        metadata={
            "help": "L2 regularization coefficient applied to the task-vector matrix."
        }
    )
    matching_loss: bool = field(
        default=False,
        metadata={
            "help": "Add query <-> key matching loss or not"
        }
    ) 
    matching_loss_v2: bool = field(
        default=False,
        metadata={
            "help": "Add query <-> key matching loss or not, v2 uses the query-key distance while v1 uses matching probablity"
        }
    ) 
    matching_loss_cls: bool = field(
        default=False,
        metadata={
            "help": "Add query <-> key (classification) matching loss or not"
        }
    ) 
    matching_loss_cls_all: bool = field(
        default=False,
        metadata={
            "help": "Add query <-> key (classification) matching loss or not, consider all keys"
        }
    ) 
    matching_loss_coeff: float = field(
        default=1,
        metadata={
            "help": "Coefficient for the matching loss term"
        }
    ) 
...
class LoraConfig(PeftConfig):
    ...
    # mpeft configs (mixture of peft modules)
    ...
    mpeft_config: KeyEncoderConfig = field(
        default=None,
        metadata={
            "help": (
                "The configuration of mixture of peft modules. If not None, will do the module composition at each LoRA layer "
            )
        },
    )
