...
from .utils import _prepare_prompt_learning_config, ModulesToSaveWrapper
...
def get_peft_model(
    ...
    # 获取模型的配置对象，若不存在则使用默认配置
    model_config = getattr(model, "config", {"model_type": "custom"})
    # 将model_config转换为字典（如果可能）
    if hasattr(model_config, "to_dict"):
        model_config = model_config.to_dict()
    # 尝试获取模型的name_or_path属性并存储到peft_config，记录基模型来源
    peft_config.base_model_name_or_path = model.__dict__.get("name_or_path", None)

    # 如果启用混合适配器，返回PeftMixedModel
    if mixed:
        return PeftMixedModel(model, peft_config, adapter_name=adapter_name)
    # 如果任务类型不支持特定映射且非Prompt Learning，返回通用PeftModel
    if peft_config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING.keys() and not peft_config.is_prompt_learning:
        return PeftModel(model, peft_config, adapter_name=adapter_name)
    
    # 如果使用Prompt Learning，调整peft_config
...
def reinit_cls_module(model, num_labels, adapter_name):
    target = model.score
    if isinstance(target, ModulesToSaveWrapper):
        reinit_cls_module = torch.nn.Linear(target.original_module.in_features, num_labels, bias=False).cuda()
        target.update(adapter_name, reinit_cls_module)
        target.set_adapter(adapter_name)
    else:
        new_module = ModulesToSaveWrapper(target, adapter_name)
        new_module.set_adapter(adapter_name)
        setattr(model, 'score', new_module)