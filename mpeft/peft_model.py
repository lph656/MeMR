...
from model.key_encoder import TaskKeyEncoder
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default") -> None:
        ...
        if self._is_prompt_learning:
            ...
            # true
            # 检查是否启用了MPEFT；检查当前适配器是否是任务列表中的第一个任务；检查是否启用了全局模块组合
            if peft_config.mpeft_enabled and adapter_name == peft_config.mpeft_config.task_list[0] and peft_config.mpeft_config.global_composition:
                # A global query encoder is needed for global module composition
                # 将key_dim设置为基础模型嵌入层的维度，确保任务键编码器与模型维度匹配
                peft_config.mpeft_config.key_dim = model.model.embed_tokens.embedding_dim
                # 实例化TaskKeyEncoder，用于MPEFT的全局模块组合。
                key_encoder = TaskKeyEncoder(peft_config.mpeft_config)
            # 获取对应的适配器模型类，并从字典中查找对应的模型类
            # mpeft.tuners.lora.model.LoraModel
            cls = PEFT_TYPE_TO_MODEL_MAPPING[peft_config.peft_type]
            # 初始化适配器模型并存储在base_model中
            self.base_model = cls(model, {adapter_name: peft_config}, adapter_name)
            # 设置额外的可训练模块
            self.set_additional_trainable_modules(peft_config, adapter_name)
            # True
            # 再次检查MPEFT和全局组合条件
            if peft_config.mpeft_enabled and peft_config.mpeft_config.global_composition:
                # True
                # 在第一个任务中设置任务键编码器
                if adapter_name == peft_config.mpeft_config.task_list[0]:
    ...
    **Attributes**:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if isinstance(self.model.active_adapter, str) and self.model.active_adapter != kwargs["active_adapter"]:
            self.model.set_adapter(kwargs["active_adapter"])
        ...
        if not peft_config.is_prompt_learning:
            ...
            if peft_config.mpeft_enabled and peft_config.mpeft_config.global_composition:
                kwargs['peft_config'] = self.peft_config # configs for all modules
...
class PeftModelForCausalLM(PeftModel):
    ...
    def __init__(self, model: torch.nn.Module, peft_config: PeftConfig, adapter_name: str = "default") -> None:
        # 调用父类PeftModel的构造函数，初始化基础模型和PEFT配置
        super().__init__(model, peft_config, adapter_name)
        self.base_model_prepare_inputs_for_generation = self.base_model.prepare_inputs_for_generation
    
    # 执行前向传播，计算logits或损失，支持LoRA、Prompt Tuning 和MPEFT
    # 处理训练和验证时的输入，根据适配器类型调整前向逻辑
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
        ...
        **kwargs,
        ...
        # 如果kwargs中提供了active_adapter，且与当前活跃适配器不同，调用set_adapter方法切换到指定的适配器
        if isinstance(self.model.active_adapter, str) and self.model.active_adapter != kwargs["active_adapter"]:
            self.model.set_adapter(kwargs["active_adapter"])
        # 获取当前活跃适配器的配置
        peft_config = self.active_peft_config
        ...
        if not peft_config.is_prompt_learning:
            ...
            if peft_config.peft_type == PeftType.POLY:
                kwargs["task_ids"] = task_ids
            # true
            # 如果启用了MPEFT且使用全局模块组合，将所有适配器的配置添加到kwargs中，供TaskKeyEncoder等模块使用
            if peft_config.mpeft_enabled and peft_config.mpeft_config.global_composition:
                kwargs['peft_config'] = self.peft_config
            # 执行非Prompt Learning的前向传播
            return self.base_model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                inputs_embeds=inputs_embeds,
    ...
    # 执行文本生成（如自回归生成对话或摘要）
    # 作用：支持推理阶段的生成，动态切换适配器，调整生成配置
    def generate(self, *args, **kwargs):
        # 在生成开始前，确保激活正确的PEFT适配器
        if isinstance(self.model.active_adapter, str) and self.model.active_adapter != kwargs["active_adapter"]:
            self.model.set_adapter(kwargs["active_adapter"])
        # 获取当前活跃适配器的PEFT配置
        peft_config = self.active_peft_config
