...
if not deepseek_api_key:
    raise RuntimeError(
        "DEEPSEEK_API_KEY is not set. Please export it before running this script."
    )
...
def score_answer(question, answer):
    try:
        ...
        response = client.chat.completions.create(
            ...
            stream=False # DeepSeek 示例中 stream=False
        )
        response_text = response.choices[0].message.content # 直接获取内容
...
def parse_score_response(response_text):
    ...
    # 修正正则表达式以更鲁棒地匹配分数，例如 "- 准确性：90/100" 或 "准确性：90/100"
    # 或者 "  - 准确性：90/100"
    score_pattern = re.compile(r'^\s*-?\s*([\u4e00-\u9fa5]+)\s*：\s*(\d+)/(\d+)\s*$') # 匹配中文名称

    for line in lines:
        line = line.strip()
        logger.debug(f"Processing line: '{line}'")
        if line.startswith("- 安全性"): # 匹配 "- 安全性："
            current_section = "安全性"
            logger.debug(f"Switched to section: 安全性")
        elif line.startswith("- 专业性"): # 匹配 "- 专业性："
            current_section = "专业性"
            logger.debug(f"Switched to section: 专业性")
        elif line.startswith("- 流畅性"): # 匹配 "- 流畅性："
            current_section = "流畅性"
            ...
            if match:
                key, score_str = match.group(1), match.group(2)
                score = int(score_str)
                logger.debug(f"Matched: section={current_section}, key={key}, score={score}")
                if key == "小计":
                    score_dict[current_section]["小计"] = score
                elif key in score_dict[current_section]:
                    score_dict[current_section][key] = score
                else:
                    logger.warning(f"Unknown key in {current_section}: {key} for line '{line}'")
            else:
                logger.debug(f"No score pattern match for line: '{line}' in section {current_section}")
            ...
            with open(output_file, 'r', encoding='utf-8') as f:
                try:
                    ...
                    if not isinstance(existing_data, list):
                        existing_data = [] # 如果文件内容不是列表，则清空
                except json.JSONDecodeError:
                    existing_data = [] # 如果文件内容无法解析，则清空
                except Exception as e:
                    logger.warning(f"Error reading existing data from {output_file}: {e}. Starting with empty list.")
                    existing_data = []
...
def process_json_file(input_file, output_file, batch_size=50):
    ...
    for i, item in enumerate(questions, 1):
        ...
        # 检查question和answer是否为空
        if not question or not answer:
            logger.warning(f"Skipping record {id_} due to empty question or answer.")
            continue
...
def calculate_average_scores(input_file, output_file):
    ...
    if not data:
        ...
        try:
            ...
            logger.info(f"Created empty average.json at {output_file} as no data was found.")
        except Exception as e:
            logger.error(f"Error creating empty average.json: {e}")
        return
    ...
    for item in data:
        try:
            # 确保所有必需的键都存在
            if all(key in item and isinstance(item[key], dict) for key in ["安全性", "专业性", "流畅性"]) and \
               all(sub_key in item["安全性"] for sub_key in ["准确性", "安全性", "伦理", "小计"]) and \
               all(sub_key in item["专业性"] for sub_key in ["理解能力", "清晰度", "个性化", "小计"]) and \
               all(sub_key in item["流畅性"] for sub_key in ["连贯性", "一致性", "温暖语气", "小计"]) and \
               "总分" in item:
                ...
                logger.warning(f"Skipping malformed record in {input_file}: {item.get('id', 'N/A')}")

        except (KeyError, TypeError) as e:
            logger.error(f"Data error in {input_file}: Record missing fields or invalid format for ID={item.get('id', 'N/A')}: {e}")
            continue
    
    if n == 0:
        logger.error(f"No valid records in {input_file} to calculate averages after filtering.")
        # 同样创建一个空的 average.json
        try:
            os.makedirs(os.path.dirname(output_file), exist_ok=True)
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump({}, f, ensure_ascii=False, indent=4)
            logger.info(f"Created empty average.json at {output_file} as no valid data was found.")
        except Exception as e:
            logger.error(f"Error creating empty average.json: {e}")
        return
...
# 循环处理所有科室的评分文件并计算均分
# 这里的 score_dir 参数应与 shell 脚本中的 OUTPUT_DIR 保持一致
def process_all_averages(score_dir="./test_infer_RAG/generate_scores_DS_v2"): # 修改默认目录
    ...
    # 动态查找 score_dir 下的所有子目录作为科室
    for item in os.listdir(score_dir):
        item_path = os.path.join(score_dir, item)
        if os.path.isdir(item_path):
            # 检查子目录中是否存在同名的.json文件
            if os.path.exists(os.path.join(item_path, f"{item}.json")):
                departments_to_process.append(item)
    
    if not departments_to_process:
        logger.warning(f"No department subdirectories with corresponding JSON files found in {score_dir}. Please ensure score files are generated.")
        return

    logger.info(f"Found departments to process for averages: {departments_to_process}")
    ...
    for dept in departments_to_process:
        ...
        logger.info(f"Processing average for {input_file}")
        calculate_average_scores(input_file, output_file)

# 验证评分数据一致性 (保持不变，用于数据质量检查)
...
def main():
    ...
    parser.add_argument("--score_dir", default="./test_infer_RAG/generate_scores_DS_v2", help="评分文件目录（均分模式）") # 修改默认目录
